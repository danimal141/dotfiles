...
`codex/wrappers/gemini-mcp.sh` (= `~/.codex/wrappers/gemini-mcp.sh` への
...
の `mise/config.toml` に out-of-store symlink される)。プロジェクト個別の
宣言は `<project>/mise.toml` または `<project>/.tool-versions` (asdf 互換)
で上書き可能。
...
| global バージョン更新 | `mise/config.toml` を直接編集 (= `vim ~/.config/mise/config.toml`) |
| LSP 等のシム再生成 | `mise reshim` |
| 利用可能バージョン一覧 | `mise ls-remote ruby` |

注意:

* `mise use -g <pkg>` は repo の `mise/config.toml` を直接書き換えるので、
...
* mise (言語ランタイム):
  * 宣言ソースは `~/.config/mise/config.toml` (= home-manager 経由 = repo の
    `mise/config.toml`)。`mise install` が読んで実体を `~/.local/share/mise/
    installs/` に展開
