...
配置する。repo の `<tool>/` 配下に raw text を置き、`home.file.<path>.source`
...
│           │                        # AGENTS.md は claude/CLAUDE.md への out-of-store symlink)
...
* **使い所**: zsh / tmux / vim / nvim / claude (CLAUDE.md / settings.json /
  ...
  apm.lock.yaml / .gitignore) / mise/config.toml / markdownlint /
  ghostty / ctags
...
* **wrapper script + `~/.codex/.env`** (codex の `GEMINI_API_KEY`):
  ...
  に env として inject する。`codex/.env.example` を template として tracked。
* **MCP server 登録 + `claude/.env`** (`GITHUB_PERSONAL_ACCESS_TOKEN` /
  `GEMINI_API_KEY` / `BRAVE_API_KEY`):
  `claude/setup-mcp.sh` が repo 内の `claude/.env` を source して
  `claude mcp add` の env として渡す。`~/.claude/.env` ではなく **repo 内の
  `claude/.env`** を読む点に注意 (= setup-mcp.sh が `cd claude && ./setup-mcp.sh`
  で実行されることを前提に `${SCRIPT_DIR}/.env` を見ている)。`claude/.env` は
  gitignore で除外、`claude/.env.example` を template として tracked。
