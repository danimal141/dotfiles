...
設計思想の詳細は `docs/design-philosophy-ja.md` を参照 (英語版は
`docs/design-philosophy.md`、両 doc 同一内容を二言語で並走させている)。
...
形式にする。判断軸は `docs/design-philosophy-ja.md` 参照。
...
* `keybindings.json` は `tools/vscode/keybindings.jsonc` への out-of-store
  symlink。`nvim ~/Library/Application\ Support/Code/User/keybindings.json`
...
* `docs/design-philosophy-ja.md` — 設計思想 (nix-darwin + home-manager 一本化方針)
* `docs/vscode-use-ja.md` / `docs/vscode-neovim-use-ja.md` — VSCode 運用
* `README-ja.md` — bootstrap / 日常運用 / 新マシン追加
