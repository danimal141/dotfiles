...
* `tools/<tool>/` — `home.file` で symlink される raw text dotfile の置き場
  (`tools/{zsh,tmux,vim,nvim,claude,codex,apm,mise,markdownlint,ghostty,ctags,vscode}/`)
...
vim と nvim は完全に分離して並行運用する。vim は legacy 環境用 (CoC +
vim-plug ベース)、nvim は新しい lua ベース構成。お互いの設定は参照しない。

* vim:
  * Plugin manager: vim-plug (`~/.vim/plugged/` は home.file 対象外、
    vim-plug 自走で書き換える領域)
  * LSP / 補完: CoC (`~/.vim/coc-settings.json`)
  * 配置: `nix/home/programs/vim.nix` が `tools/vim/.vimrc` と
    `tools/vim/.vim/{coc-settings.json,filetype.vim,autoload,colors,ftdetect,ftplugin}`
    を個別 symlink (`~/.vim/plugged/` 等の動的領域を保護するため一括 symlink
    は避ける)
* nvim:
  ...
  * Tree-sitter: nvim-treesitter (master または main branch、`tools/nvim/lua/plugins/treesitter.lua`)
